...
// Find a todo with the indicated ID in the indicated todoList. Returns
...
// Render individual todo list and its todo items
...
app.post("/lists/:todoListId/destroy", (req, res, next) => {
  let todoLists = req.session.todoLists;
  let todoListId = +req.params.todoListId;
  let todoListIndex = todoLists.findIndex(todoList => todoList.id === todoListId);
...
app.post("/lists/:todoListId/edit", 
  [
    body("todoListTitle")
      .trim()
      .isLength({ min: 1 })
      .withMessage("New todo list title required.")
      .isLength({ max: 100 })
      .withMessage("The new todo list title must be between 1 and 100 characters.")
      .custom((title, { req }) => {
        let todoLists = req.session.todoLists;
        let duplicate = todoLists.find(list => list.title === title);
        return duplicate === undefined;
  ...
  (req, res, next) => {
    ...
    if (todoList === undefined) {
      ...
      if (!errors.isEmpty()) {
        ...
        res.render(`/lists/${req.params.todoListId}/edit`, {
          flash: req.flash(),
          todoListTitle: req.body.todoListTitle,
          todoList: todoList,
        });
